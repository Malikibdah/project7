async function ShowCartDelail() {
    ...
    let url1 = `https://localhost:7208/api/Order/GetCart/${userid}`;
...
async function placeorder() {
    ...
    let url2 = "https://localhost:7208/api/Order/GetUserAddress";
...
const url = "https://localhost:7208/api/Order/AddNewOrderByUserId";
...
async function getlastorderid() {
    let userid =localStorage.getItem("Userid");
    const url = `https://localhost:7208/api/Order/GetLastOrderIdByUserId/${userid}`; 
    let response = await fetch(url);
    let data = await response.json();
    let orderid = data.id
...
const url = `https://localhost:7208/api/Payment/AddToPaymentTableByOrderId/${orderid}`;
let paymentamount ={paymentAmount:localStorage.getItem('TotalPrice')};
let response = await fetch(url, {
    method : 'POST',
